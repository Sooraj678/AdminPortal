	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ("admin370@gmail.com".equals(email) && "admin123".equals(password) && "Admin".equals(loginRole)) {
			...
			try {
				...
				response.sendRedirect("admin/adminHome.jsp?msg=valid");
				System.out.println("Into Admin Home Page:");
			} catch (IOException e) {
			...
			try {
				...
				ResultSet rs = st.executeQuery("select *from usersadminportal where email = '" + email
						+ "' and password = '" + password + "' and loginRole = '" + loginRole + "'");

				while (rs.next()) {
					HttpSession session = request.getSession(true);
					z = 1;
					session.setAttribute("email", email);
					response.sendRedirect("studentHome.jsp?msg=valid");
				}
				if (z == 0) {
			...
			try {
				...
				while (rs.next()) {
					...
					z = 1;
					session.setAttribute("email", email);
					response.sendRedirect("teacherHome.jsp?msg=valid");
				}
				if (z == 0) {
					response.sendRedirect("login.jsp?msg=notExist");
				}
